...
describe('RetrievalService', () => {
  ...
  it('calls retrieve with default params and maps results', async () => {
    ...
    expect(retrieve).toHaveBeenCalledWith('what is RAG', 'kb-1', expect.objectContaining({
      topK: 10,
      minScore: 0.7,
      useReranker: false,
      denseWeight: 0.5,
  ...
  it('uses dto params when provided', async () => {
    ...
    const dto = { query: 'test', kbId: 'kb-1', topK: 5, minScore: 0.8, useReranker: true, denseWeight: 0.3 };
  ...
  it('records usage log on success', async () => {
    ...
    expect(usageLog.record).toHaveBeenCalledWith(expect.objectContaining({
      type: 'retrieval',
      kbId: 'kb-1',
      status: 'success',
