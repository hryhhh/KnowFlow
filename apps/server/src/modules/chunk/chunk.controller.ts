...
export class ChunkController {
  ...
  async findByDoc(
    ...
    const res = await this.service.findByDoc(docId, parseInt(pageSize, 10), parseInt(page, 10));
  ...
  async findByKb(
    ...
    const res = await this.service.findByKb(kbId, parseInt(pageSize, 10), parseInt(page, 10));
    return { code: 0, data: res };
  }

  @Post('documents/:docId/chunks')
  async create(@Param('docId') docId: string, @Body() body: { content: string; title?: string }) {
